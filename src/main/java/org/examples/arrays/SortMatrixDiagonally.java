 ...
 * [[1,1,1,1],[1,2,2,2],[1,2,3,3]] Example 2:
 * 
 * Input: mat =
...
public class SortMatrixDiagonally {
	...
	public int[][] diagonalSort(int[][] mat) {
		Map<Integer, PriorityQueue<Integer>> diagonalElems = new HashMap<>();
		for (int i = 0; i < mat.length; i++) {
			for (int j = 0; j < mat[0].length; j++) {
				diagonalElems.putIfAbsent(i - j, new PriorityQueue<Integer>());
				diagonalElems.get(i - j).add(mat[i][j]);
			}
		}

		for (int i = 0; i < mat.length; i++) {
			for (int j = 0; j < mat[0].length; j++) {
				mat[i][j] = diagonalElems.get(i - j).poll();
			}
		}
