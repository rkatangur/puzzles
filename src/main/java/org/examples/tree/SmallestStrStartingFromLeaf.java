...
public class SmallestStrStartingFromLeaf {
	...
	private String smallestStr = null;

	public String smallestFromLeaf(TreeNode root) {
		StringBuilder sb = new StringBuilder();
		smallestFromLeafHelper(root, sb);
		return smallestStr;
	}

	public void smallestFromLeafHelper(TreeNode root, StringBuilder sb) {
		...
		if (root.left == null && root.right == null) {
			...
			if (smallestStr == null) {
				smallestStr = curStr;
			} else if (smallestStr.compareTo(curStr) > 0) {
				smallestStr = curStr;
			}
		}

		smallestFromLeafHelper(root.left, sb);
		smallestFromLeafHelper(root.right, sb);
